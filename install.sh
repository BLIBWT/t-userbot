...
runin() {
  ...
  { "$@" 2>>../t-userbot-install.log || return $?; } | while read -r line; do
    spin
    printf "%s\n" "$line" >> ../t-userbot-install.log
  done
}
...
touch t-userbot-install.log
if [ ! x"$SUDO_USER" = x"" ]; then
  chown "$SUDO_USER:" t-userbot-install.log
fi
...
if [ -d "t-userbot/t-userbot" ]; then
  cd t-userbot || { endspin "Failed to chdir"; exit 6; }
  DIR_CHANGED="yes"
fi
if [ -f ".setup_complete" ] || [ -d "t-userbot" -a ! x"" = x"$DYNO" ]; then
  # If t-userbot is already installed by this script, or its in Heroku and installed
  PYVER=""
...
echo "Installing..." > t-userbot-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
  ...
  endspin "Unrecognised OS. Please follow https://BLIBWT.github.io/installing_advanced"
  exit 1
fi
...
${SUDO_CMD}"python$PYVER" -m t-userbot "$@" || { echo "Python scripts failed"; exit 5; }
