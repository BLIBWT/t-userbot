...
class CloudBackend():
    ...
    async def _find_data_channel(self):
        async for dialog in self._client.iter_dialogs(None, ignore_migrated=True):
            if dialog.name == f"user-{self._me.id}-data" and dialog.is_channel:
    ...
    async def _make_data_channel(self):
        async with self._anti_double_lock:
            if not self._data_already_exists:
                ...
                return (await self._client(CreateChannelRequest(f"user-{self._me.id}-data",
    ...
    async def _find_asset_channel(self):
        async for dialog in self._client.iter_dialogs(None, ignore_migrated=True):
            if dialog.name == f"user-{self._me.id}-assets" and dialog.is_channel:
                members = await self._client.get_participants(dialog, limit=2)
                if len(members) != 1:
                    continue
